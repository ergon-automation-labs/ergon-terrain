defmodule BotArmyTerrain.Repo.Migrations.CreateCards do
  ...
  def change do
    create table(:cards, primary_key: false, schema: "terrain") do
      add :id, :binary_id, primary_key: true
      add :track_id, references(:tracks, type: :binary_id, on_delete: :cascade), null: false
      add :chunk_id, references(:content_chunks, type: :binary_id, on_delete: :set_null)
    ...
    create index(:cards, [:track_id], schema: "terrain")
    create index(:cards, [:next_review_at], schema: "terrain")
    create index(:cards, [:state], schema: "terrain")
    create unique_index(:cards, [:track_id, "md5(front)"], schema: "terrain", name: :idx_cards_front_hash)
  end
end
